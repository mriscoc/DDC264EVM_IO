...
partial class Program
{
    ...
    int CONV_LOW_INT = 1600;
    int CONV_HIGH_INT = 1600;
    int CONV_CONFIG = 0; // 0-Freerun, 1-Freerun, 2-Low, 3-High
    const int CLC_HIGH = 7;
    const int CLC_LOW = 7;
    const int DDC_CLK_CONFIG = 1; // 1=Running, 0=disabled;
    int CHANNEL_COUNT = 16;
    int NDVALID_IGNORE = 255;
    int NDVALID_READ = 256;
    ...
    byte CFGLOW = 0;
    byte CFGHIGH = 7; // Range b11, 20 bits

    int USBdev = 0; // DDC EVM board to send/receive data to/from

    // Register arrays
    static int regsSize = 255;
    ...
    static void Main()
    {
        ...
        double[] AllData = new double[program.CHANNEL_COUNT * 2 * program.NDVALID_READ];

        program.DDC_Adquisition(ref AllData[0]);

        // Show captured data
        for (int i = 0; i < program.NDVALID_READ; i++)
        {
